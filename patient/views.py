...
def editPatient(request):
    profile = get_object_or_404(PatientProfile, patient=request.user)
    ...
    if request.method == "POST":
        ...
        profile.Aadhar_Number = request.POST.get("Aadhar_Number", profile.Aadhar_Number)
        ...
        profile.save()
        return redirect('patient:patientProfile')  # Redirect to the profile page

    return render(request, 'patient/patient-profile-edit.html', {'profile': profile})
